...
export class CreateLinkView extends TemplateView {
    render(t, vm) {
        ...
        return t.div({className: "CreateLinkView card"}, [
            t.h1("Create shareable links to Wepoc rooms, users or messages."),
            t.form({action: "#", onSubmit: evt => this._onSubmit(evt)}, [
                t.div(t.input({
                    ...
                    name: "identifier",
                    required: true,
                    placeholder: "#room:wepoc.co, @user:wepoc.co",
    ...
    _onIdentifierChange(evt) {
        ...
        if (!this.value.validateIdentifier(inputField.value)) {
            inputField.setCustomValidity("That doesn't seem valid. Try #room:wepoc.co, @user:wepoc.co or +group:wepoc.co.");
        } else {
